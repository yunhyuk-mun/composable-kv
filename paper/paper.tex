...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\textwidth]{pareto.pdf}
\caption{Quality--cost trade-off across composition methods. Held-out next-token KL (Table~\ref{tab:holdout} aggregate) versus composition wall-clock (Table~\ref{tab:cost}) on Qwen-2.5-0.5B, CPU, $|B|=19$. Green arrow: M1 $\to$ M4 (naive + H3). Purple arrow: M2 $\to$ M5 (RoPE + H3). At either cost point, adding H3 moves the operating point downward at approximately fixed compute (composition-only view; setup assumes $\mathrm{KV}_A, \mathrm{KV}_B$ already computed and in memory).}
\label{fig:pareto}
\end{figure}
