...
\title{Composable KV: Decomposing Independent Prefill Cache Composition Errors\\ into Position and Cross-Context Mismatch, and Partially Closing the Latter\\ with a Lightweight Value Corrector}

\author[1]{Yoonh Moon\thanks{\texttt{moonyoonh91@gmail.com}}}
...
\begin{abstract}
Large language model serving increasingly relies on caching independent prefix key--value (KV) representations for reuse across queries. Composing two independently prefilled caches, however, yields degraded next-token behavior compared to a full joint prefill. We formalize this degradation as the sum of a \emph{position mismatch} (rotary encoding assumes each token's absolute index) and a \emph{cross-context representation mismatch} (each cache never attended to the other), and study the two error sources separately on Qwen-2.5-0.5B. Positional alignment via RoPE-shift improves top-1 agreement with the full-prefill baseline in three of four controlled conditions but does not consistently reduce Kullback--Leibler divergence to the oracle, demonstrating that the two behavioral metrics can diverge. A layer-selective RoPE-shift (skipping mid layers where the per-layer value cosine drops most) yields no improvement over full RoPE-shift, suggesting the mid-layer dip is a symptom of missing cross-context information rather than an intervention target on its own. We introduce a single $64\times64$ linear map applied to the middle-layer value cache, trained by ridge-regularized least squares to predict the full-prefill value from the independent value. Across four cross-validated folds at the example level, the corrector reduces held-out downstream next-token KL by 6.4\% on naive concatenation and 9.4\% on RoPE-shifted concatenation, matching in-sample performance within $3\times10^{-3}$ KL. The correction adds approximately $0.34$ ms of wall-clock and $\sim 0.0021\%$ of the FLOPs of one full $B$-prefill. All numbers are supported by a set of pipeline sanity checks (cache round-trip KL $\approx 3\times10^{-7}$, RoPE-shift-by-zero is exact identity, prefill is deterministic, and the $A$-portion of the joint cache is bit-exact with the standalone $A$ prefill within float32 precision). We frame our result as a proof of feasibility on a small controlled benchmark, with concrete follow-ups toward scale, model families, and realistic serving workloads.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
    \item We formalize the composition error decomposition, and design four controlled conditions (independent, referential, conflicting, cross-inferential) that vary the degree to which $B$ semantically depends on $A$.
...
\subsection{Held-out generalization of H3}\label{sec:holdout}
...
The held-out corrector matches its in-sample counterpart within $3\times10^{-3}$ KL, indicating that the linear map generalizes across held-out examples rather than memorizing the training set. On the conflicting condition, M5 achieves top-1 agreement $1.00$ on held-out (every held-out example's composed distribution places the same token first as the full-prefill oracle).
...
\begin{table}[t]
\centering
\caption{Wall-clock cost of composition on CPU, relative to a single $B$-prefill forward. H3 correction is a $64\times64$ matmul at a single layer.}
\label{tab:cost}
\begin{tabular}{lrr}
\toprule
...
We decomposed the error of independent-prefill KV composition into a position component and a cross-context representation component, and showed on a small controlled benchmark that a single $64\times64$ linear map on the mid-layer value cache reduces held-out downstream next-token KL by $6.4$--$9.4\%$ across all four condition types. The corrector generalizes from 16 training to 4 held-out examples per fold within $3\times10^{-3}$ KL, and adds approximately $0.34$ ms and $0.0021\%$ of the FLOPs of one $B$-prefill. Scaling the benchmark, reproducing on larger and more diverse models, and testing more expressive correctors are the natural next steps.

\paragraph{Code and reproduction.} All code, data conditions, raw result logs, sanity checks, and cost measurements are released at \url{https://github.com/<repo-to-be-added>/composable-kv} under an MIT license.
...
Because the observed downstream method deltas (Table~\ref{tab:main}) are on the order of $10^{-2}$ KL, they are approximately $10^5$ times the round-trip noise floor, and their attribution to composition behavior (rather than injection artifacts) is safe.
